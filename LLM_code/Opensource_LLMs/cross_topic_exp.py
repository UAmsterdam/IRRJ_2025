...
def llm_code(data, models, prompt, topic, cross_topic):
    print(f'Processing topic: {topic} with cross topic: {cross_topic}, Data size: {data.shape[0]}')
    output_dir = './few_shot_results_cross_topic'
    ...
    for model_name in models:
        ...
        for i, row in data.iterrows():
            print(f"Running: {i+1}/{len(data)} for {topic} (Cross topic: {cross_topic}), {model_name}")
            try:
            ...
            results.append({
                ...
                "Answer": answer,
                "Topic": topic,
                "Cross Topic": cross_topic
            })
        all_results.extend(results)
    
    final_data = pd.DataFrame(all_results)
    output_filename = os.path.join(output_dir, f'FSL_output_results_all_models_{topic}_cross_topic_w_{cross_topic}.csv')
...
cross_topics = ['1086', '1244', '1247']

for topic in topics:
    for cross_topic in cross_topics:
        print(f"Running for Topic: {topic} with Cross Topic: {cross_topic}")
        df_train, df_test = train_test_data(df, '../../core/qrels/', cross_topic)
        prompt = prompt_generation(data_desc, df_train, cross_topic)
        ...
        llm_code(final_data, models, prompt, topic, cross_topic)